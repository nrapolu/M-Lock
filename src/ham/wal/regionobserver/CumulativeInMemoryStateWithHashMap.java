...
public class CumulativeInMemoryStateWithHashMap {

	private static final Log LOG = LogFactory
			.getLog(CumulativeInMemoryStateWithNavigableMap.class);
	private final HRegionInfo regionInfo;

	private static final String SEPARATOR = "@";
	private ConcurrentHashMap<String, ConcurrentNavigableMap<String, TreeMap<Long, KeyValue>>> myInMemStore = new ConcurrentHashMap<String, ConcurrentNavigableMap<String, TreeMap<Long, KeyValue>>>(
	...
	public CumulativeInMemoryStateWithHashMap(HRegionInfo regionInfo) {
		super();
	...
	public void addPut(final Put write) {
		...
		for (byte[] family : familyMap.keySet()) {
			...
			for (KeyValue kv : kvList) {
				...
				ConcurrentNavigableMap<String, TreeMap<Long, KeyValue>> colMap = myInMemStore
						.get(row);
				if (colMap == null) {
					colMap = new ConcurrentSkipListMap<String, TreeMap<Long, KeyValue>>();
					myInMemStore.put(row, colMap);
				}
				String colKey = familyStr + SEPARATOR + colStr;
				TreeMap<Long, KeyValue> tsMap = colMap.get(colKey);
				if (tsMap == null) {
					tsMap = new TreeMap<Long, KeyValue>();
					colMap.put(colKey, tsMap);
				}
				sysout("InMemStore##: Putting at timestamp: " + kv.getTimestamp()
						+ ", is this kv: " + kv.toString());
				tsMap.put(kv.getTimestamp(), kv);
			}
		}
	}
		...
		for (Map.Entry<byte[], NavigableSet<byte[]>> scanFamMapEntry : scanFamMap
			...
			if (scanFamMapEntry.getValue() != null
				...
				for (byte[] col : scanFamMapEntry.getValue()) {
					String colKey = famStr + SEPARATOR + Bytes.toString(col);
					TreeMap<Long, KeyValue> timestampVal = colMap.get(colKey);
